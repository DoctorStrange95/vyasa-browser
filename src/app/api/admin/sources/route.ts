...
async function getIDSPHistory(req: Request) {
  ...
  if (ageHours >= IDSP_TTL_HOURS) {
    try {
      ...
      await fetch(`${base}/api/idsp-weekly?force=1`, { cache: "no-store" });
    } catch { /* non-fatal */ }
  }

  // List all week snapshots (keyed as YYYY_wWW)
  const all = await adminList("idsp_weekly", 60);
  let weeks = all
    .filter(d => /^\d{4}_w\d{2}$/.test(String(d._id)))
    .sort((a, b) => String(b._id).localeCompare(String(a._id))); // newest first

  // Backfill: if no historical docs yet, force a fresh fetch to create the first one
  if (weeks.length === 0 && cached) {
    try {
      const base = new URL(req.url).origin;
      await fetch(`${base}/api/idsp-weekly?force=1`, { cache: "no-store" });
      const all2 = await adminList("idsp_weekly", 60);
      weeks = all2
        .filter(d => /^\d{4}_w\d{2}$/.test(String(d._id)))
...
export async function GET(req: Request) {
  ...
  const [phi, submissions, waitlist, idspHistory, feedback, users] = await Promise.allSettled([
    Promise.all([
  ...
  return NextResponse.json({
    ...
    idsp:        idspHistory.status  === "fulfilled" ? idspHistory.value : [],
